...
class SareesController extends Controller
{
	...
	public function productSort(Request $request)
	{
		...
		$sel_color = $this->sel_color;
		$color = Colors::get();
		$query = $this->getProductsSortByProductType($sort_by);
	...
	public function productColor(Request $request)
	{
		...
		$query = $this->getProductsColorByProductType($sel_color);
	...
	public function CategoryDetails(Request $request)
	{
		...
		$query = $this->getAllProducts($category_id);

		$shop_all_products = $query->inRandomOrder()->orderBy('id', 'DESC')->get();
		$color = Colors::get();
		$category = Category::get();
		//  Load More Product with Ajax
		if ($request->ajax()) {
	...
	// {
	// 	$category = Product::where('category_id',$request->category_name)->get();
	// 	$color = $category->where('color_id',$request->color_sort)->get();

	// 	return view('shop_details',compact('category','color'));
	// }
}
